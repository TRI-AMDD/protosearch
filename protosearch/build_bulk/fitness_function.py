...
from catkit.gen.utils.connectivity.coordinates import expand_cell
...
def get_area_neighbors(atoms, cell_cutoff=15, cutoff=None):
    ...
    connectivity_int = np.zeros((len(atoms), len(atoms)), dtype=int)
    try:
    ...
    for i, n in enumerate(origional_indices):
        ...
        for j, edge in enumerate(unique_edge):
            ...
            for ii, ind in enumerate(ridge_indices[indices]):
                ...
                area = get_weighted_area(vertices)
                if area is None:
                    continue
