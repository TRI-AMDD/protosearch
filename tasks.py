...
This is a slightly modified version of of S. P. Ong's pymatgen
administration tasks.py, see https://github.com/materialsproject/pymatgen
"""
...
from protosearch import __version__ as CURRENT_VER
...
def set_ver(ctx):
    lines = []
    with open("protosearch/__init__.py", "rt") as f:
        for l in f:
            if l.startswith("__version__"):
                lines.append('__version__ = "%s"' % NEW_VER)
            else:
                lines.append(l.rstrip())
    with open("protosearch/__init__.py", "wt") as f:
        f.write("\n".join(lines))
...
def release_github(ctx):
    ...
    response = requests.post(
        "https://api.github.com/repos/ToyotaResearchInstitute/protosearch/releases",
...
def release(ctx, notest=False, nover=False):
    """
    Run full sequence for releasing protosearch.

    :param ctx:
    :param notest: Whether to skip tests.
    :param notest: Whether to skip autoversion (e. g. if tagging version).
    """
    ctx.run("rm -r dist build protosearch.egg-info", warn=True)
    if not nover:
        set_ver(ctx)
    if not notest:
